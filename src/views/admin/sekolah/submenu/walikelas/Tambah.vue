...
const doStoreData = async (d) => {
  ...
  try {
    const response = await Api.post(
      `owner/datasekolah/${id}/walikelas`,
      dataStore
    );
...
<template>
  <div class="pt-4 px-10 md:flex justify-between">
    ...
    <div class="md:py-0 py-4">
      <BreadCrumb>
        <template v-slot:content> GURU BK
                ...
                <div class="w-full mx-0">
                  <div class="bg-white rounded-lg p-0 sm:p-6 xl:p-0">
                    <div class="grid md:grid-cols-2 gap-2">
                      ...
                      <div>
                        <label for="name" class="text-sm font-medium text-gray-900 block mb-2">NIP</label>
                        <Field v-model="dataDetail.nomerinduk" :rules="validateData" type="text" name="nomerinduk"
                          ref="nomerinduk" class="input input-bordered md:w-full max-w-2xl" required />
                        <div class="text-xs text-red-600 mt-1">
                          {{ errors.nomerinduk }}
                        </div>
                      </div>
