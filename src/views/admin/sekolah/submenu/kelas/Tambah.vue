...
//   try {
//     const response = await Api.get(`owner/datasekolah/${id}/kelas/${id2}`);
//     dataDetail.value = {
...
const doStoreData = async (d) => {
  ...
  try {
    const response = await Api.post(
      `owner/datasekolah/${id}/kelas`,
      dataStore
    );
    Toast.success("Success", "Data Berhasil ditambahkan!");
    // resetForm();
    router.push({ name: "AdminSekolahDetailKelas", params: { id } });
                ...
                <div class="w-full mx-0">
                  <div class="bg-white rounded-lg p-0 sm:p-6 xl:p-0">
                    <div class="grid md:grid-cols-2 gap-2">
                      ...
                      <div>
                        <label for="name" class="text-sm font-medium text-gray-900 block mb-2">Walikelas</label>
                        <v-select class="py-2 px-3 w-72 mx-auto md:mx-0" :options="pilihWalikelas"
                          v-model="dataDetail.walikelas_id" v-bind:class="{ disabled: false }"></v-select>
                        <div class="text-xs text-red-600 mt-1">
                          {{ errors.nama }}
                        </div>
                      </div>
                      <div>
                        <label for="name" class="text-sm font-medium text-gray-900 block mb-2">Guru Bk</label>
                        <v-select class="py-2 px-3 w-72 mx-auto md:mx-0" :options="pilihBk" v-model="dataDetail.bk_id"
