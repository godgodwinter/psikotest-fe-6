...
storeAdminBar.setPagesActive("buletin");
...
const getDataDetail = async () => {
  try {
    const response = await Api.get(`owner/buletinpsikologi/${id}`);
...
const doStoreData = async (d) => {
  ...
  try {
    const response = await Api.post(`owner/buletinpsikologi/${id}`, dataStore);
...
const fnDoUploadFile = async (file, jenis) => {
  let link = `owner/buletinpsikologi/${id}`;
    ...
    <div class="md:py-0 py-4">
      <BreadCrumb>
        <template v-slot:content>
          Buletin Psikologi <BreadCrumbSpace /> Edit
        </template>
      </BreadCrumb>
    </div>
  </div>
