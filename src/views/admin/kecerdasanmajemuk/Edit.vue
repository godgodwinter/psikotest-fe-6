...
const getDataDetail = async () => {
  try {
    const response = await Api.get(`owner/kecerdasanmajemuk/${id}`);
...
const doStoreData = async (d) => {
  ...
  try {
    const response = await Api.put(`owner/kecerdasanmajemuk/${id}`, dataStore);
    ...
    <div class="md:py-0 py-4">
      <BreadCrumb>
        <template v-slot:content>
          Kecerdasan Majemuk <BreadCrumbSpace /> Edit
        </template>
      </BreadCrumb>
    </div>
  </div>
