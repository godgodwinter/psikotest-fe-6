...
const doResetSalah = async (kelas_siswa_id) => {
  if (confirm("Apakah anda mereset waktu data ini?")) {
    // console.log(kelas_siswa_id);
    try {
      const response = await Api.post(`admin/menuujian/proesssiswa/${kelas_siswa_id}/reset_salah`);
      console.log(response);
      Toast.babeng('Reset waktu dan Jawaban Salah berhasil!',)
      // return response.data;
    } catch (error) {
      console.error(error);
    }
